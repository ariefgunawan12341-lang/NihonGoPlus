...
const MODE_PROMPTS: Record<string, string> = {
  ...
  'ssw-interview': 'This is a SSW (Specified Skilled Worker) VISA INTERVIEW simulation. Ask about work experience, availability, and basic workplace Japanese relevant to the stated industry, in polite Japanese.',
...
export async function onRequest({ request, env }: { request: Request; env: { ANTHROPIC_API_KEY?: string } }) {
  ...
  if (!apiKey) {
    return new Response(
      JSON.stringify({ error: 'ANTHROPIC_API_KEY is not configured on the server. Add it in your Cloudflare Pages environment variables.' }),
      { status: 500 }
    )
  }
